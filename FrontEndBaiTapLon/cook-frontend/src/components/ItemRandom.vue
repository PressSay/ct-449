...
<template>
    <!-- add "d-md-none d-lg-block" when 'screen is medium' -->
    <div class="col-lg-4 col-md-6 col-sm-12">
        <div class="product-box-layout1">
            <figure class="item-figure"><a :href="recipeUrl(this.recipe._id)"><img style="width: auto; height: 327px;"
                        :src="recipeImgUrl(this.images ?? null, this.backend)" alt="image ran"></a>
            </figure>
            <div class="item-content">
                <span class="sub-title" :v-model="this.category.name">{{ this.category.name }}</span>
                <h3 class="item-title"><a :href="recipeUrl(this.recipe._id)"
                    :v-model="this.recipe.name">{{ this.recipe.name }}</a></h3>
