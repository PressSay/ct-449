...
export default {
    ...
    watch: {
        isMounted() {
            this.$emit("isRenderedF", this.isMounted && this.last);
        }
    },
    mounted() {
        this.refresh();
...
<template>
    <div class="ranna-slider-content-layout1">
        <figure class="item-figure"><a :href="recipeUrl(this.recipe._id)" ><img :src="recipeImgUrl(this.images ?? null, this.backend)" alt="Recipe"></a>
        </figure>
        <div class="item-content">
            <span class="sub-title" :v-model="this.category.name">{{ this.category.name }}</span>
            <h2 class="item-title"><a :href="recipeUrl(this.recipe._id)" :v-model="this.recipe.name">{{ this.recipe.name }}</a></h2>
            ...
            <ul class="entry-meta">
                ...
                <li><a :href="'#'"><i class="fas fa-user"></i>by <span :v-model="this.author.name">{{ this.author.name }}</span></a></li>
