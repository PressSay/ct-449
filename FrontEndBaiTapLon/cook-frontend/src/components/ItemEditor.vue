...
<template>
    <!-- remove "d-md-none d-lg-block" when it not three element -->
    <div class="col-lg-4 d-block d-md-none d-lg-block col-sm-12 col-12">
        <div class="product-box-layout2">
            <figure class="item-figure"><a :href="recipeUrl(this.recipe._id)"><img
                        style="width: 370px; height: 399px;"
                        :src="recipeImgUrl(this.images ?? null, this.backend)" alt="item editor"></a></figure>
            <div class="item-content">
                <span class="sub-title" :v-model="this.category.name">{{ this.category.name }}</span>
                <h3 class="item-title"><a :href="recipeUrl(this.recipe._id)" :v-model="this.recipe.name">{{ this.recipe.name
                }}</a></h3>
                ...
                <ul class="entry-meta">
                    ...
                    <li><a href="#"><i class="fas fa-heart"></i><span :v-model="this.recipe.like">{{ this.recipe.like
                    }}</span> Likes</a></li>
                </ul>
            </div>
        </div>
